...
export const api = {
  ...
  effectiveConfig: () => request("/config/effective"),
  resetConfigToEnv: () => request("/config/reset-to-env", { method: "POST" }),
  updateConfig: (payload: Partial<BotConfig>) => request<BotConfig>("/config", { method: "PATCH", body: JSON.stringify(payload) }),
  activatePresentationMode: () => request("/presentation/activate", { method: "POST" }),
  marketStatus: () => request("/market/status"),
  replayStatus: () => request("/replay/status"),
